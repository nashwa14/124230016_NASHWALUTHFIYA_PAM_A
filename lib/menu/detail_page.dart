...
class _DetailItemPageState extends State<DetailItemPage> {
  ...
  Widget _buildMainInfoCard(InventoryItem item) {
    ...
    if (daysRemaining <= 0) {
      ...
      status = '🟡 Hampir Habis ($daysRemaining hari)';
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _infoRow('Status', status, Icons.info, color: statusColor),
          ],
        ),
      ),
    );
  ...
  Widget _buildConversionCard(Map<String, double> rates, Map<String, int> offsets) {
    ...
    // Ambil rates yang dibutuhkan (IDR sudah diasumsikan base)
    final usdRate = rates['USD'] ?? 0.000064; 
    final eurRate = rates['EUR'] ?? 0.000059;
    final jpyRate = rates['JPY'] ?? 0.0096;

    // Kita hitung harga dalam IDR untuk konversi
    // Asumsi: Kita mencari faktor konversi dari mata uang item ke IDR
    final double conversionFactor = 1.0 / (rates[widget.item.currency] ?? 1.0); 
    final double priceInIDR = price * conversionFactor; // Harga item di-normalize ke IDR
